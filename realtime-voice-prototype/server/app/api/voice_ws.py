...
from app.session.turn_controller import TurnAction, TurnController, TurnControllerAction
...
NO_TRANSCRIPT_REPLY = "I did not catch that turn clearly. Could you say it again?"
EMPTY_ASSISTANT_REPLY = "I could not generate a useful follow-up. Could you add one concrete example?"
...
async def voice_session(websocket: WebSocket) -> None:
    ...
    async def finalize_current_turn(reason: str) -> None:
        ...
        await cancel_assistant("new user turn finalized")
    ...
    try:
        while True:
            ...
            if "bytes" in message and message["bytes"] is not None:
                ...
                has_voice, _, _ = voice_activity.inspect(audio_bytes)
                if has_voice:
                    await cancel_assistant("user started speaking")
                    turn_controller.observe_audio(has_voice=True)
                else:
                    turn_controller.observe_audio(has_voice=False)
                if not turn_controller.active:
                    continue
                try:
                    updates = await asr.push_audio(audio_bytes)
                except Exception as error:
                    await send_json(session_error(f"Speech recognition failed: {error}"))
                    continue
            ...
            if event is ClientEvent.START:
                await cancel_assistant("session restarted")
            ...
            if event is ClientEvent.STOP:
                await cancel_assistant("session stopped")
                await cancel_turn_monitor()
